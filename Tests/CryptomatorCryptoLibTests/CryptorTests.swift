...
class CryptorTests: XCTestCase {
	...
	func testEncryptAndDecryptStream() throws {
		...
		try Cryptor.copyStream(inputStream: cryptor.decryptInputStream(wrapped: InputStream(data: encryptedData)),
	...
	func testEncryptAndDecryptContentStream() throws {
		...
		// encrypt content
		try Cryptor.copyStream(inputStream: InputStream(url: originalURL)!,
		                       outputStream: cryptor.encryptOutputStream(wrapped: OutputStream(url: ciphertextURL, append: false)!))

		// decrypt content
		try Cryptor.copyStream(inputStream: cryptor.decryptInputStream(wrapped: InputStream(url: ciphertextURL)!),
